...
class ViewModelRecords : ViewModel() {
    ...
    fun insertActivitySession(genericActivity: List<GenericActivity>) {
        viewModelScope.launch(Dispatchers.IO) {
            val uuid = getUUID().toString()

            genericActivity.forEach(
                fun(activity: GenericActivity) {
                    Log.d("Supabase", "Inserimento della sessione: $activity")
                    ...
                    safeRpcCall("insert_${activity.activityEnum.name.lowercase()}_session", jsonFinal)
                    Log.d("Supabase", "Sessione inserita con successo: $activity")
                }
            )
        }
    }
